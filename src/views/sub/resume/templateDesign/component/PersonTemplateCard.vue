...
const emit = defineEmits(['toDesign', 'deletePersonTemplate', 'copyUserResume'])
...
// 点击删除简历
async function deleteUserResume() {
  ElMessageBox.confirm('删除该创作后无法恢复，确定继续？', '警告', {
    ...
    .then(async () => {
      emit('deletePersonTemplate', props.cardData.id)
    })
    .catch(() => {})
}
  ...
  <div class="card-wraper" @mouseover="mouseover" @mouseleave="mouseleave">
    <div class="template-card-box">
      ...
      <div ref="maskLayerRef" class="mask-layer">
        <div class="icon-box">
          <!-- 复制 -->
          <div class="copy-box" @click="copyUserResume">
            <ReIcon icon="i-ep:copy-document" color="#fff" size="20px" />
          </div>
          <!-- 删除 -->
          <div class="delete-box" @click="deleteUserResume">
            <ReIcon icon="i-ep:delete" color="#fff" size="20px" />
          </div>
        </div>
