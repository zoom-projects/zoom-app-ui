...
// 选择背景图
function handleClickImg(item: { src: string }) {
  if (item.src) {
    HJSchemaJsonStore.css.backgroundImage = getAssetsImagesFile(item.src)
  }
  else {
    HJSchemaJsonStore.css.backgroundImage = ''
  }
}
</script>
    ...
    <div class="img-list-box">
      <template v-for="(item, index) in bgcImgList" :key="index">
        <div class="img-box">
          ...
          <div v-else class="no-img" @click="handleClickImg(item)">
            空
          </div>
        </div>
      </template>
