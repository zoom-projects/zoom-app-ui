...
<template>
  <el-form>
    ...
    <el-form-item>
      <div flex>
        <el-radio v-model="radioValue" :value="4" style="margin-right: 10px;">
          指定
        </el-radio>
        <el-checkbox-group v-model="checkboxList" class="checkbox" @change="() => radioValue = 4">
          <el-checkbox v-for="item in monthList" :key="item.key" :label="item.key">
            {{ item.value }}
          </el-checkbox>
        </el-checkbox-group>
      </div>
